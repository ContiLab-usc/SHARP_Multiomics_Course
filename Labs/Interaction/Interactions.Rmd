...
$H0:\beta_{GxE} = 0$ is equivalent to testing $\frac{OR_{GxE}}{OR_G \times OR_E} = 1$ e.g. departure from multiplicative scale interaction    
...
model_control_only <- glm(e3_alcpreg_yn_None ~ SNP.1 + h_age_None + h_mbmi_None +  h_cohort + h_edumc_None + e3_sex_None + ethn_PC1 + ethn_PC2, family = binomial, data = dataset_controlonly)

kable(tidy(model_control_only), digits = 2) %>% 
  kable_styling(bootstrap_options = c("striped", "hover"), full_width = F, position = 'left')
```
